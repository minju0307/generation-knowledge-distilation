...
def validate(tokenizer, model, device, loader):
  ...
  with torch.no_grad():
      for _, data in enumerate(loader, 0):
          ...
          story=[tokenizer.decode(i, skip_special_tokens=True, clean_up_tokenization_spaces=True) for i in ids]
  ...
  return stories, predictions, actuals

def T5Test(dataframe, source_text, target_text, model_params, output_dir="./outputs/" ):
    ...
    story, predictions, actuals = validate(tokenizer, model, device, val_loader)
    final_df = pd.DataFrame({'story': story, 'Generated Text': predictions, 'Actual Text': actuals})
...
if __name__=='__main__':
    ...
    for i in range(0, data_num, args.few_shot ):
        ...
        for j in range(i, i+args.few_shot):
            ...
            if not j == (i + args.few_shot - 1):
                prompt= prompt +''+story+ ''+ending
            else:
                prompt = prompt+''+story+ ''

        prompt = prompt+"\n continue write the story: "
    ...
    output_dir='t5_few_shot_'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    T5Test(dataframe=df2, source_text="story", target_text="ending", model_params=model_params, output_dir=output_dir)