...
if __name__=='__main__':
    ...
    cli_parser.add_argument("--epoch", type=int, required=True)
    cli_parser.add_argument("--batch", type=int, required=True)
    cli_parser.add_argument("--sampling", type=float, required=True)
    ...
    #load dataset
    df = pd.read_csv(f"data/story_cloze_old/10shot_story_cloze_ver2_for_pretrain2.csv", sep=',')
    df=df[:10]
    df=df[['src_text', 'target_text']]
    # if cli_args.sampling <1:
    #     values = df.values.tolist()
    #     sampling=random.sample(values, int(len(df)*cli_args.sampling))
    #     new_df={'src_text': [], 'target_text': []}
    #     for src, trg in sampling:
    #         new_df['src_text'].append(src)
    #         new_df['target_text'].append(trg)
    #     df=pd.DataFrame(new_df)
    # df["src_text"] = df["src_text"]
    # df["target_text"] = df["target_text"]

    #load test dataset
    df2 = pd.read_csv(f"data/story_cloze_old/10shot_story_cloze_ver2_for_pretrain2.csv", sep=',')
    df2=df2
    df2=df2[['src_text', 'target_text']]
    print(df2.head(10))
    # df2["src_text"] = df2["src_text"]
    # df2["target_text"] = df2["target_text"]
    ...
    model_params = {
        ...
        "TRAIN_EPOCHS": cli_args.epoch,  # number of training epochs
    ...
    output_dir=f'{cli_args.model}_{cli_args.task}_10shot_2_{cli_args.epoch}'
